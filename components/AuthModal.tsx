...
export default function AuthModal({ visible, onClose }: AuthModalProps) {
  ...
  async function performAutoCloudRestore(userId: string) {
    try {
      const cloudHabits = await restoreHabitsFromCloud(userId);
      ...
      Alert.alert(
        "Welcome Back",
        "Your garden and profile have been automatically restored. 🌱💦",
      );
    } catch (restoreErr) {
      console.log("Failed auto-restoring data on login:", restoreErr);
    }
  }
        ...
        <View className="bg-zinc-900/90 border border-zinc-800 rounded-[32px] p-6 shadow-2xl">
          <View className="items-center mb-6">
            ...
            <Text className="text-zinc-400 text-sm mt-1 text-center">
              {isLogin
                ? "Log in to backup your garden to the cloud."
                : "Create an account to keep your habits safe."}
            </Text>
          </View>
          ...
          <TouchableOpacity
            className="absolute top-6 right-6 p-2 bg-zinc-800/80 rounded-full"
            onPress={onClose}
          >
